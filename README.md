...
- **WAV/MP3形式の音声ファイル対応**
- アラート重要度別の音（低・中・高・緊急）
- 操作フィードバック音（成功・エラー・情報）
- システム起動音
- ワンクリックでミュート/アンミュート切り替え
- 音声ファイルのキャッシュ機能

**音声ファイルの準備**: [音声ファイル配置ガイド](docs/SOUND_FILES_SETUP.md) を参照してください。

詳細は **[通知音機能ガイド](docs/SOUND_NOTIFICATION.md)** を参照してください。

### テストコントロールパネル 🎮
実際のロボットがなくてもシステムをテストできます！
- 手動でダミーアラートを送信
- ロボット状態を更新
- ランダムアラート生成
- クイックアクション（緊急アラート、バッテリー充電など）
- リアルタイム送信ステータス確認

詳細は **[テストコントロールパネル使用ガイド](docs/TEST_CONTROL_PANEL.md)** を参照してください。
...
│   └── TestControlPanel.vue   # テストコントロールパネル (NEW!)
├── services/
...
初回接続時: **約 22KB** (正常)
- 実データ: 約 2KB
- Firebase オーバーヘッド: 約 20KB

2回目以降: **約 0.5KB〜2KB** (差分更新のみ)

月間推定使用量: **約 130MB** (無料枠10GBの1.3%)

詳細は [Firebase使用量ガイド](docs/FIREBASE_USAGE.md) を参照してください。
